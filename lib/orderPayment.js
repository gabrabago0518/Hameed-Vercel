...
// captured here.
//
// By request, a successful online payment skips CONFIRMED entirely and
// lands the order straight on PREPARING — the kitchen starts on it
// immediately rather than waiting for a staff member to notice a "Confirmed"
// order and click "Mark preparing" themselves. Both history rows are still
// recorded (CONFIRMED immediately followed by PREPARING) so the customer
// tracker's "Order Confirmed" stage still gets a real, accurate timestamp —
// this only changes what Order.status ends up as, not the audit trail.
export async function markOrderPaid(orderId, paymongoPaymentId = null) {
  const result = await prisma.$transaction(async (tx) => {
    ...
    await tx.orderStatusHistory.create({
      data: { orderId, status: "PREPARING", note: "Automatically moved to preparing" },
    });
  ...
  // Only one email goes out (PREPARING, the order's real resulting status) —
  // not a separate CONFIRMED email first, since both transitions happen in
  // the same instant here and two emails a second apart would just be noise.
  if (result.changed) {
    await notifyOrderStatusChange(orderId, "PREPARING");
  }
  return result;
}
...
// this one records who verified it and when. Same skip-CONFIRMED behavior as
// markOrderPaid above, for the same reason and by the same request — once a
// COD order is verified, it goes straight to PREPARING rather than sitting
// as "Confirmed" waiting for a separate "Mark preparing" click. This is also
// what keeps the staff Pending tab meaning exactly one thing (awaiting a
// confirmation call) instead of two different things.
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    await tx.orderStatusHistory.create({
      data: {
        orderId,
        status: "PREPARING",
        note: "Automatically moved to preparing",
        changedByUserId: verifiedByUserId,
      },
    });
...
// Triggered by staff/admin clicking "Unreachable" on a Cash on Delivery
// order awaiting confirmation, each time a call attempt fails to reach the
// customer. Capped at 3 — the 3rd attempt cancels the order outright rather
// than leaving it stuck forever awaiting a call nobody can complete (mirrors
// how markOrderPaymentExpired/markOrderPaymentFailed above cancel an online
// order once it's clear it's never going to be paid). Payment.status moves
// to FAILED on that final attempt, same as any other payment that never
// materialized — nothing was ever collected for a COD order either way.
export async function markOrderUnreachable(orderId, staffUserId) {
  const result = await prisma.$transaction(async (tx) => {
    const payment = await tx.payment.findUnique({ where: { orderId } });
    ...
    const attempts = payment.unreachableAttempts + 1;
    const cancelling = attempts >= 3;
    ...
    if (cancelling) {
      ...
      await tx.orderStatusHistory.create({
        data: {
          orderId,
          status: "CANCELLED",
          note: "Cancelled — customer unreachable after 3 call attempts",
      ...
      await tx.orderStatusHistory.create({
        data: {
          ...
          note: `Customer unreachable (attempt ${attempts} of 3)`,
          changedByUserId: staffUserId,
        },
      });
    }

    return { changed: true, attempts, cancelled: cancelling };
  });
...
// the customer side. A refund also moves Order.status to REFUNDED, treated
// the same as CANCELLED everywhere that matters (see lib/orderStatus.js) —
// a refunded order is done, regardless of how far it got through
// fulfillment beforehand.
export async function refundOrderPayment(orderId, refundedByUserId = null) {
  ...
  await prisma.$transaction([
    ...
    prisma.orderStatusHistory.create({
      data: {
        ...
        changedByUserId: refundedByUserId,
      },
    }),
  ]);
...
// Also moves Order.status to REFUNDED, same as refundOrderPayment — there's
// no admin to attribute this one to (PayMongo triggered it), so the history
// entry's changedByUserId stays null, same as the other webhook/poll-driven
// transitions above (markOrderPaid, markOrderPaymentFailed, etc.).
export async function markOrderPaymentRefunded(orderId, paymongoRefundId = null) {
  ...
  await prisma.$transaction([
    ...
    prisma.order.update({ where: { id: orderId }, data: { status: "REFUNDED" } }),
    prisma.orderStatusHistory.create({
      data: { orderId, status: "REFUNDED", note: "Refund confirmed by PayMongo" },
    }),
  ]);
