...
// A QR/GCash order the customer never actually paid for within the
// 15-minute window (markOrderPaymentExpired in lib/orderPayment.js is what
// flips it here, once the poll route or the scheduled sweep notices —
// isPaymentWindowExpired above is the "hasn't been noticed yet" transitional
// state, a different thing). By request, treated as an abandoned cart, not
// a real order: it doesn't count as "an order" in any history/volume view
// (customer order history, staff/admin order lists, order counts) even
// though the row itself is kept, never hard-deleted, so it's still reachable
// by direct link if a support conversation ever needs it. A CANCELLED order
// with any other Payment.status (FAILED from an active decline, still
// PENDING from a manual admin cancel, or no payment row at all) is a real
// cancellation and stays visible everywhere as normal.
export function isAbandonedExpiredOrder(order) {
  return order.status === "CANCELLED" && order.payment?.status === "EXPIRED";
}

// The same rule as isAbandonedExpiredOrder above, as a Prisma `where`
// fragment — for queries (counts, list queries) that filter at the database
// level rather than fetching rows to test in JS. Spread this into any
// `where` that lists or counts "orders" so an abandoned cart never counts.
export const EXCLUDE_ABANDONED_EXPIRED_ORDERS_WHERE = {
  NOT: { status: "CANCELLED", payment: { status: "EXPIRED" } },
};
...
// A "regular" transition is either a no-op, cancelling/refunding (both
// always allowed directly — distinct escape hatches, not a "skip"; a refund
// is treated the same as cancelling here), or the single next step
// getNextOrderStatus would take. Anything else (a multi-stage jump, a
// backward move, or un-cancelling/un-refunding) needs the admin to explicitly
// confirm the override on /admin/orders/[id] — see setOrderStatusAction.
...
// once CANCELLED/REFUNDED — those get their own messaging (see
// PaymentSection) instead of a tracker with nothing meaningful to show.
